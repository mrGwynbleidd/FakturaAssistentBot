...
from app.core.approve_action_logger import log_approve_action
...
#returns dict with csv_written, index_updated, error — used by approve_review_case for logging
def save_to_approved_cases(
    ...
    try:
        ...
        index_error = f"index_error: {err}"
        error = f"{error}; {index_error}".strip("; ")

    return {"csv_written": csv_written, "index_updated": index_updated, "error": error}


#approves a review case: saves to approved.csv, updates chroma, marks case as approved in csv
#logs the full approve action including csv/index status to approved_actions.csv
...
def approve_review_case(
    ...
    source_id = case.get("source_id", "") or case_id

    save_result = save_to_approved_cases(
        case_id=case_id,
        question=question,
    ...
    status = "ok" if (save_result["csv_written"] and save_result["index_updated"]) else "partial"
    ...
    try:
        ...
        log_approve_action(
            ...
            source_id=source_id,
            action="approve",
            approved_csv_written=save_result["csv_written"],
            index_updated=save_result["index_updated"],
            status=status,
