...
def format_review_case(row: dict, index: int) -> str:
    return(
        f"{index}. Case Id: {row.get('case_id', '')}\n"
        f"Причина: {row.get('reason', '')}\n"
        f" Вопрос: {row.get('question', ''), 300}\n"
        f"Ответ бота: {row.get('bot_answer', ''), 300}"
    )



@router.message(F.text.in_(text_values("btn_review_cases")))
async def show_review_cases(message: Message):
    ...
    parts= [get_admin_text("review_cases_title", language)]

    for index, case in enumerate(cases, start=1):
        parts.append(format_review_case(case, index))

    parts.append(
        "\nКоманда:\n"
        "/approve_case case_id - одобрить и ввести правильный ответ\n"
        "/reject_case case_id - отклонить"
    )
...
async def approve_case_command(
    ...
    await state.update_data(case_id)
